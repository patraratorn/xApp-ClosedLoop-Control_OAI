...
void log_int_value(const char *name_str, const label_info_lst_t label_info, const meas_record_lst_t meas_record)
{
  ...
  if (label_info.noLabel != NULL) {
    ...
    snprintf(payload, sizeof(payload), "{\"meas_name\": \"%s\", \"value\": %d, \"unit\": \"%s\"}", name_str, meas_record.int_val, name_unit);
    if (zmq_pub) zmq_send(zmq_pub, payload, strlen(payload), 0);
    ...
    snprintf(payload, sizeof(payload), "{\"meas_name\": \"%s\", \"binX\": %d, \"binY\": %d, \"binZ\": %d, \"value\": %d, \"unit\": \"%s\"}", name_str, *label_info.distBinX, *label_info.distBinY, *label_info.distBinZ, meas_record.int_val, name_unit);
    if (zmq_pub) zmq_send(zmq_pub, payload, strlen(payload), 0);
  }
}
...
void log_real_value(const char *name_str, const label_info_lst_t label_info, const meas_record_lst_t meas_record)
{
  ...
  snprintf(payload, sizeof(payload), "{\"meas_name\": \"%s\", \"value\": %.2f, \"unit\": \"%s\"}", name_str, meas_record.real_val, name_unit);
  if (zmq_pub) zmq_send(zmq_pub, payload, strlen(payload), 0);
}
...
int main(int argc, char* argv[])
{
  ...
  printf("ZMQ Publisher bound to tcp://*:5555\n");
